...
import static com.commerce.user.infrastructure.persistence.mapper.UserDynamicSqlSupport.defaultAddressId;
...
public interface UserDynamicMapper {
	...
	BasicColumn[] selectList = BasicColumn.columnList(
			id, firstName, secondName, phone, email, defaultAddressId, deleted, createTime
	);
	...
	@Results(id = "UserDataResult", value = {
			...
			@Result(column = "default_address_id", property = "defaultAddressId", jdbcType = JdbcType.BIGINT),
	...
	default int create(UserData userData) {
		return MyBatis3Utils.insert(this::insert, userData, users, c -> c
				...
				.map(defaultAddressId).toPropertyWhenPresent("defaultAddressId", userData::defaultAddressId)
	...
	default int update(UserData userData) {
		return MyBatis3Utils.update(this::updateStatement, users, c -> c
				...
				.set(defaultAddressId).equalToWhenPresent(userData::defaultAddressId)
	...
	default List<UserData> query(UserQueryOptions options) {
		...
		return MyBatis3Utils.selectList(this::selectMany, selectList, users, c -> c
				...
				.and(defaultAddressId, isEqualToWhenPresent(() -> safeOptions.getDefaultAddressId().orElse(null)))
